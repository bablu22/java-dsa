# Course Progress Tracker

Track your completion status for each topic. Update the status as you progress.

**Status Legend:**
- `[ ]` Not started
- `[~]` In progress
- `[x]` Completed
- `[R]` Under revision

---

## Module 1 — Foundations

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 01 | Variables & Data Types | [ ] | [ ] | [ ] | [ ] | |
| 02 | Operators | [ ] | [ ] | [ ] | [ ] | |
| 03 | Conditional Statements | [ ] | [ ] | [ ] | [ ] | |
| 04 | Loops | [ ] | [ ] | [ ] | [ ] | |
| 05 | Patterns | [ ] | [ ] | [ ] | [ ] | |
| 06 | Functions & Methods | [ ] | [ ] | [ ] | [ ] | |
| 07 | OOP | [ ] | [ ] | [ ] | [ ] | |

---

## Module 2 — Arrays & Strings

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 08 | Arrays | [ ] | [ ] | [ ] | [ ] | |
| 09 | 2D Arrays | [ ] | [ ] | [ ] | [ ] | |
| 10 | Strings | [ ] | [ ] | [ ] | [ ] | |

---

## Module 3 — Core Algorithms

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 11 | Basic Sorting Algorithms | [ ] | [ ] | [ ] | [ ] | |
| 12 | Bit Manipulation | [ ] | [ ] | [ ] | [ ] | |
| 13 | Time & Space Complexity | [ ] | [ ] | [ ] | [ ] | |

---

## Module 4 — Recursion & Algorithms

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 14 | Recursion | [ ] | [ ] | [ ] | [ ] | |
| 15 | Divide & Conquer | [ ] | [ ] | [ ] | [ ] | |
| 16 | Backtracking | [ ] | [ ] | [ ] | [ ] | |

---

## Module 5 — Linear Data Structures

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 17 | ArrayLists | [ ] | [ ] | [ ] | [ ] | |
| 18 | Linked Lists | [ ] | [ ] | [ ] | [ ] | |
| 19 | Stacks | [ ] | [ ] | [ ] | [ ] | |
| 20 | Queues | [ ] | [ ] | [ ] | [ ] | |
| 30 | Collections Framework | [ ] | [ ] | [ ] | [ ] | List, Set, Map, Queue, Deque, Iterator, Comparator |

---

## Module 6 — Trees

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 21 | Binary Trees | [ ] | [ ] | [ ] | [ ] | |
| 22 | Binary Search Trees | [ ] | [ ] | [ ] | [ ] | |
| 23 | Heaps | [ ] | [ ] | [ ] | [ ] | |

---

## Module 7 — Graphs

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 24 | Greedy Algorithms | [ ] | [ ] | [ ] | [ ] | |
| 25 | Graphs | [ ] | [ ] | [ ] | [ ] | |

---

## Module 8 — Hashing & Tries

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 26 | Hashing | [ ] | [ ] | [ ] | [ ] | |
| 27 | Tries | [ ] | [ ] | [ ] | [ ] | |

---

## Module 9 — Dynamic Programming

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 28 | Dynamic Programming | [ ] | [ ] | [ ] | [ ] | |

---

## Module 10 — Advanced Topics

| # | Topic | Theory | Code | Practice | Status | Notes |
|---|-------|--------|------|----------|--------|-------|
| 29 | Segment Trees | [ ] | [ ] | [ ] | [ ] | |

---

## Summary

| Module | Total Topics | Completed | Progress |
|--------|-------------|-----------|----------|
| 1. Foundations | 7 | 0 | 0% |
| 2. Arrays & Strings | 3 | 0 | 0% |
| 3. Core Algorithms | 3 | 0 | 0% |
| 4. Recursion & Algorithms | 3 | 0 | 0% |
| 5. Linear Data Structures | 5 | 0 | 0% |
| 6. Trees | 3 | 0 | 0% |
| 7. Graphs | 2 | 0 | 0% |
| 8. Hashing & Tries | 2 | 0 | 0% |
| 9. Dynamic Programming | 1 | 0 | 0% |
| 10. Advanced | 1 | 0 | 0% |
| **Total** | **30** | **0** | **0%** |

---

## Practice Problem Log

Track solved problems here for quick review.

| # | Problem | Platform | Topic | Difficulty | Date | Notes |
|---|---------|----------|-------|------------|------|-------|
| | | | | | | |
